...
if ('navigation' in window) {
  ...
  const STORE_HISTORY   = 'animowebHistory';        // recently watched, used to seed recommendations
  ...
  async function tmdb(path, params={}) {
    ...
    // A transient network blip or a bad JSON parse used to cache `null`
    // forever — every future call for that exact URL (e.g. this title's
    // detail page) would keep resolving null for the rest of the session,
    // with no way to recover short of a full page reload. Failures are no
    // longer cached, so the next call for the same key gets a fresh retry.
    const p = fetch(key).then(r => r.json()).catch(() => null);
    cache.set(key, p);
    p.then(result => { if (result == null) cache.delete(key); });
    return p;
  }
  ...
  // Lightweight "recently watched" log — separate from the resume/progress
  // store above (which only tracks ONE position per id, driven by 3rd-party
  // player postMessages and is unreliable for anime). This just remembers,
  // per browser, which titles were opened on a watch page and when, so we
  // can use it as a signal for "because you watched…" recommendations even
  // when the embedded player never reports progress.
  function getHistory() {
    try { return JSON.parse(localStorage.getItem(STORE_HISTORY) || '[]'); } catch { return []; }
  }
  function logHistory(entry) {
    ...
    try {
      ...
      hist = hist.filter(h => `${h.type}:${h.id}` !== key);
      hist.unshift(Object.assign({}, entry, { ts: Date.now() }));
      if (hist.length > HISTORY_MAX) hist = hist.slice(0, HISTORY_MAX);
      localStorage.setItem(STORE_HISTORY, JSON.stringify(hist));
    } catch (_) {}
  }
  ...
  function pagerHtml(p, pages, start, end, total, dataAttr) {
    ...
    return `
      <nav class="ep-pager" aria-label="Episode pages">
        ...
        <form class="ep-pager__jump" data-jump="${dataAttr}">
          <input type="number" min="1" max="${total}" inputmode="numeric" class="ep-pager__jump-input" placeholder="Go to episode #" aria-label="Go to episode number" />
          <button type="submit" class="ep-pager__jump-btn" title="Go to episode" aria-label="Go to episode"><i class="ri-arrow-right-line"></i></button>
        </form>
      </nav>`;
  }

  // Wires the "go to episode" jump form rendered by pagerHtml(). `total` is
  // the number of episodes being paginated, `perPage` is EPISODES_PER_PAGE,
  // `gotoPage(page, epNum)` should render the requested page — after it
  // runs we scroll to + briefly flash whichever card carries the matching
  // `data-epnum`, so jumping feels precise even though it lands on a page,
  // not a single row.
  function wireEpisodeJump(grid, dataAttr, total, perPage, gotoPage) {
    if (!grid) return;
    const form = grid.querySelector(`.ep-pager__jump[data-jump="${dataAttr}"]`);
    if (!form) return;
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      const input = form.querySelector('.ep-pager__jump-input');
      const n = parseInt(input.value, 10);
      if (!Number.isFinite(n) || n < 1 || n > total) {
        input.classList.add('ep-pager__jump-input--err');
        setTimeout(() => input.classList.remove('ep-pager__jump-input--err'), 500);
        return;
      }
      const targetPage = Math.max(1, Math.min(Math.ceil(total / perPage), Math.ceil(n / perPage)));
      gotoPage(targetPage, n);
      input.value = '';
      requestAnimationFrame(() => {
        const card = grid.querySelector(`[data-epnum="${n}"]`);
        if (card) {
          try { card.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (_) {}
          card.classList.add('episode--jumped');
          setTimeout(() => card.classList.remove('episode--jumped'), 1600);
        } else {
  ...
  function renderEpisodePage(eps, seasonNum, showId, page, activeSeasonNum, activeEpisodeNum) {
    ...
    const cards = slice.map(ep => {
      ...
      return `
      <a class="episode${isPlaying ? ' episode--playing' : ''}" href="/watch/tv/${showId}/${seasonNum}/${ep.episode_number}" data-link data-epnum="${ep.episode_number}">
  ...
  function wireEpisodes(data, activeSeasonNum, activeEpisodeNum) {
    ...
    const renderPage = (page) => {
      ...
      _scrollToPlayingEp();
      wireEpisodeJump(grid, 'go', currentEps.length, EPISODES_PER_PAGE, (targetPage) => renderPage(targetPage));
    };
  ...
  // ───── Related content (watch pages) ──────────────────────────────────────
  // Builds a deduped list of "you may also like" titles for a movie/TV watch
  // page: TMDB's own recommendations for the title being watched, topped up
  // with recommendations drawn from a couple of recently-watched OTHER
  // titles (from history) so the rail still has depth even when TMDB has
  // few recommendations for a niche title.
  async function buildRelatedTitles(type, id) {
    const seen = new Set([`${type}:${id}`]);
    const items = [];
    const addAll = (results, fallbackType) => {
      (results || []).forEach(r => {
        if (!r || !r.poster_path) return;
        const mt = r.media_type || fallbackType;
        const key = `${mt}:${r.id}`;
        if (seen.has(key)) return;
        seen.add(key);
        items.push({ ...r, media_type: mt });
      });
    };

    try {
      const recs = await tmdb(`/${type}/${id}/recommendations`).catch(() => null);
      addAll(recs?.results, type);
    } catch (_) {}

    if (items.length < 16) {
      try {
        const hist = getHistory()
          .filter(h => (h.type === 'movie' || h.type === 'tv') && `${h.type}:${h.id}` !== `${type}:${id}`)
          .slice(0, 3);
        for (const h of hist) {
          if (items.length >= 16) break;
          const recs2 = await tmdb(`/${h.type}/${h.id}/recommendations`).catch(() => null);
          addAll(recs2?.results, h.type);
        }
      } catch (_) {}
    }
  ...
  function relatedSectionHtml(items, heading) {
    if (!items || !items.length) return '';
    return `
      <section class="section watch-related">
        <header class="section__head"><div><h2 class="section__title">${heading || 'You may also <em>like</em>'}</h2></div></header>
        <div class="rail">${items.map(r => titleCard(r)).join('')}</div>
      </section>`;
  }

  // Same idea, but for anime — uses AniList's own recommendation edges for
  // the title being watched, topped up from a couple of recently-watched
  // OTHER anime in history.
  async function buildRelatedAnime(anilistId) {
    const seen = new Set([Number(anilistId)]);
    const items = [];
    const RECS_Q = `query($id:Int){
      Media(id:$id, type:ANIME){
        recommendations(sort: RATING_DESC, perPage: 12){
          nodes{ mediaRecommendation{ id title{romaji english} coverImage{extraLarge large medium} format averageScore seasonYear } }
        }
      }
    }`;
    const addNodes = (nodes) => {
      (nodes || []).forEach(n => {
        const m = n && n.mediaRecommendation;
    ...
    try {
      const data = await anilistGql(RECS_Q, { id: Number(anilistId) }).catch(() => null);
      addNodes(data?.Media?.recommendations?.nodes);
    } catch (_) {}

    if (items.length < 16) {
      try {
        const hist = getHistory()
          .filter(h => h.type === 'anime' && Number(h.id) !== Number(anilistId))
          .slice(0, 2);
        for (const h of hist) {
          if (items.length >= 16) break;
          const data2 = await anilistGql(RECS_Q, { id: Number(h.id) }).catch(() => null);
          addNodes(data2?.Media?.recommendations?.nodes);
        }
      } catch (_) {}
    }
  ...
  function animeRelCard(m) {
    const poster = (m.coverImage && (m.coverImage.extraLarge || m.coverImage.large || m.coverImage.medium)) || '';
    const title  = (m.title && (m.title.english || m.title.romaji)) || 'Unknown';
    const isFilm = m.format === 'MOVIE';
    const rating = m.averageScore ? `<span class="title-card__rating"><i class="ri-star-fill"></i> ${(m.averageScore/10).toFixed(1)}</span>` : '';
    ...
    return `<a class="title-card" href="/watch/anime/${m.id}/1" data-link>
      <div class="title-card__poster"><div class="title-card__shimmer"></div>${art}${rating}${badge}</div>
      <div class="title-card__meta">
        <div class="title-card__title">${html(title)}</div>
        <div class="title-card__sub">${m.seasonYear || '—'}</div>
      </div>
    </a>`;
  }

  function relatedAnimeSectionHtml(items) {
    if (!items || !items.length) return '';
    return `
      <section class="section watch-related">
        <header class="section__head"><div><h2 class="section__title">You may also <em>like</em></h2></div></header>
        <div class="rail">${items.map(animeRelCard).join('')}</div>
      </section>`;
  }
  ...
  async function renderWatch(...args) {
    ...
    // Episode list (TV) — episode picker only; related content is handled
    // below for both movie and TV via buildRelatedTitles().
    if (type === 'tv') {
    ...
    // Related content — TMDB recommendations for the title being watched,
    // topped up with recommendations drawn from recently-watched history.
    // Movies additionally get a single "Up next" pick in the aside; both
    // movie and TV watch pages get the full "You may also like" rail
    // (previously TV had no related content at all, and movies only ever
    // showed the ONE "up next" card unless the user opened the title page).
    buildRelatedTitles(type, id).then(items => {
      if (type === 'movie') {
        const nextUpEl = $('#nextUp');
        const first = items[0];
        if (nextUpEl) {
          nextUpEl.innerHTML = first ? `
            <a class="continue-card" style="flex:1;background:var(--bg-1)" href="/watch/${first.media_type || 'movie'}/${first.id}${first.media_type === 'tv' ? '/1/1' : ''}" data-link>
              <div class="continue-card__art">
                ${first.backdrop_path ? `<img src="${IMG('w780', first.backdrop_path)}" loading="lazy">` : ''}
                <div class="continue-card__play"><i class="ri-play-circle-fill"></i></div>
              </div>
              <div class="continue-card__body">
                <div class="continue-card__title">${html(first.title || first.name)}</div>
                <div class="continue-card__meta"><span>Up next</span></div>
              </div>
            </a>` : '<p style="color:var(--text-mute);font-size:13px">No suggestions just yet.</p>';
        }
      }
      const watchPageEl = $('.watch-page');
      if (watchPageEl && items.length) {
        watchPageEl.insertAdjacentHTML('beforeend', relatedSectionHtml(items));
      }
    }).catch(() => {});
  ...
  async function renderAnimeWatch(anilistId, episodeNum) {
    ...
    logHistory({
      id, type: 'anime', title,
      cover: media.coverImage?.large || media.bannerImage || null,
      year,
      episode: ep,
    });
    ...
    $('.watch-page__info').innerHTML = `
      ...
      <aside class="watch-page__aside">
        <h4>What's next</h4>
        <div id="nextUp"><p style="color:var(--text-mute);font-size:13px">Loading suggestions…</p></div>
      </aside>
    `;
    ...
    if (watchPage) {
      ...
      const renderEpsFor = (anilistId, eps, currentEp, label, page) => {
        ...
        const cards = Array.from({ length: end - start }, (_, i) => {
          ...
          return `
            <a class="episode episode--compact${playingCls}" href="/watch/anime/${anilistId}/${n}" data-link data-epnum="${n}">
        ...
        wireEpisodeJump(grid, 'eplistgo', eps, EPISODES_PER_PAGE, (targetPage) => renderEpsFor(anilistId, eps, currentEp, label, targetPage));
      };
          ...
          const renderSeasonEps = async (season, page) => {
            ...
            const cards = slice.map(e => {
              ...
              const thumb = e.still_path
                ? `<img src="${IMG('w300', e.still_path)}" loading="lazy">`
                : `<div class="episode__still-fallback"><i class="ri-film-line"></i></div>`;
              const meta = [
                e.air_date ? `<span>${fmtDate(e.air_date)}</span>` : '',
                e.runtime  ? `<span class="dot"></span><span>${e.runtime} min</span>` : '',
                e.vote_average > 0 ? `<span class="dot"></span><span><i class="ri-star-fill" style="color:var(--accent)"></i> ${fmtScore(e.vote_average)}</span>` : ''
              ].filter(Boolean).join('');
              return `
                <a class="episode${isCurrent ? ' episode--playing' : ''}" href="/watch/anime/${aniId}/${aniEp}" data-link data-epnum="${n}">
                  <div class="episode__still">
                    ${thumb}
                    <span class="episode__num">E${n}</span>
                    <span class="episode__playhint"><i class="ri-play-fill"></i></span>
                    <button class="episode__dl btn-dl" data-dl-type="tv" data-dl-id="${tmdbId}" data-dl-s="${sn}" data-dl-e="${n}" title="Download" aria-label="Download episode ${n}"><i class="ri-download-cloud-2-line"></i></button>
                  </div>
                  <div class="episode__body">
                    <div class="episode__title">${html(e.name || 'Episode ' + n)}</div>
                    <div class="episode__meta">${meta}</div>
                    <div class="episode__overview">${html(e.overview || 'No summary available.')}</div>
                  </div>
                </a>`;
            }).join('');
            ...
            wireEpisodeJump(grid, 'seasongo', tmdbEps.length, EPISODES_PER_PAGE, (targetPage) => renderSeasonEps(season, targetPage));
    ...
    // Related anime — AniList's own recommendation edges for this title,
    // topped up from recently-watched history, so both the aside pick and
    // the full rail below the episode list have real suggestions instead of
    // relying on the user clicking into "similar titles" separately.
    buildRelatedAnime(id).then(items => {
      const nextUpEl = $('#nextUp');
      const first = items[0];
      if (nextUpEl) {
        nextUpEl.innerHTML = first ? `
          <a class="continue-card" style="flex:1;background:var(--bg-1)" href="/watch/anime/${first.id}/1" data-link>
            <div class="continue-card__art">
              ${(first.coverImage?.extraLarge || first.coverImage?.large) ? `<img src="${first.coverImage.extraLarge || first.coverImage.large}" loading="lazy">` : ''}
              <div class="continue-card__play"><i class="ri-play-circle-fill"></i></div>
            </div>
            <div class="continue-card__body">
              <div class="continue-card__title">${html(first.title?.english || first.title?.romaji || 'Unknown')}</div>
              <div class="continue-card__meta"><span>Suggested</span></div>
            </div>
          </a>` : '<p style="color:var(--text-mute);font-size:13px">No suggestions just yet.</p>';
      }
      const watchPageEl = $('.watch-page');
      if (watchPageEl && items.length) {
        watchPageEl.insertAdjacentHTML('beforeend', relatedAnimeSectionHtml(items));
      }
    }).catch(() => {});
    ...
    $('#searchResults').innerHTML = `
      ...
      ${people.length ? `<div class="group"><h4>People · ${people.length}</h4><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px">${people.slice(0, 12).map(p => `
        <a class="s-person" href="/person/${p.id}" data-link onclick="OMNIFLIX.closeSearch()">
          <div class="s-person__photo">${p.profile_path ? `<img src="${IMG('w185', p.profile_path)}" loading="lazy">` : ''}</div>
