  ...
  class StellarPlayer {
    ...
    _resolveAnimeMapping(anilistId) {
      ...
      const p = fetchJsonWithRetry(`https://api.ani.zip/mappings?anilist_id=${anilistId}`)
        .then(j => {
          ...
          const episodes = (j && j.episodes) || {};
          // AniZip occasionally has a gap for a specific episode number (no
          // seasonNumber on that one entry) even though every other episode
          // of this AniList entry maps cleanly to the same TMDB season —
          // this is what caused the intermittent "shows S1E1 but actually
          // plays a different episode" bug for AniList entries that are
          // themselves season 2+ of a show. Instead of defaulting a missing
          // mapping to season 1, find the season number every OTHER episode
          // agrees on (the mode) and use that as the fallback.
          const seasonCounts = Object.create(null);
          Object.keys(episodes).forEach(k => {
            const sn = episodes[k] && episodes[k].seasonNumber;
            if (sn == null) return;
            seasonCounts[sn] = (seasonCounts[sn] || 0) + 1;
          });
          let dominantSeason = null, bestCount = 0;
          Object.keys(seasonCounts).forEach(sn => {
            if (seasonCounts[sn] > bestCount) { bestCount = seasonCounts[sn]; dominantSeason = parseInt(sn, 10); }
          });
          return {
            tmdb: tmdb || null,
            imdb: m.imdb_id || null,
    ...
    _mount() {
      ...
      if (this.ctx.type === 'anime' && isMovieKind) {
        ...
        const seasonN  = (epMap.seasonNumber  != null) ? epMap.seasonNumber  : (map.dominantSeason != null ? map.dominantSeason : 1);
